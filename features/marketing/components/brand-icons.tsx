...
/**
 * Brand icon components with SVG paths sourced from simple-icons.
 * Paths are hardcoded here to avoid Turbopack barrel-import issues
 * with the simple-icons CJS package.
 */
...
interface RawIconProps extends BrandIconProps {
  path: string;
  fill?: string;
  viewBox?: string;
}

function RawIcon({ path, fill = 'currentColor', viewBox = '0 0 24 24', size = 16, className }: RawIconProps) {
  return (
    <svg
...
// SVG path data from simple-icons v16
const PATHS = {
  github:
...
export function GitHubIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.github} fill="#ffffff" size={size} className={className} />;
}

export function CursorIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.cursor} fill="#ffffff" size={size} className={className} />;
}

export function GitHubCopilotIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.githubCopilot} fill="#ffffff" size={size} className={className} />;
}

export function ClaudeCodeIcon({ size = 16, className }: BrandIconProps) {
  // Anthropic orange works well on dark surfaces
  return <RawIcon path={PATHS.claudeCode} fill="#D97757" size={size} className={className} />;
}

export function WindsurfIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.windsurf} fill="#ffffff" size={size} className={className} />;
}

export function JetBrainsIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.jetbrains} fill="#ffffff" size={size} className={className} />;
}

export function VSCodeIcon({ size = 16, className }: BrandIconProps) {
  // VSCodium icon (visually identical to VS Code), rendered in VS Code blue
  return <RawIcon path={PATHS.vsCode} fill="#4fc3f7" size={size} className={className} />;
}

export function AnthropicIcon({ size = 16, className }: BrandIconProps) {
  return <RawIcon path={PATHS.anthropic} fill="#D97757" size={size} className={className} />;
}

/** MCP is a protocol — use a minimal radial/network icon. */
export function McpIcon({ size = 16, className }: BrandIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
...
const ICON_MAP: Record<string, React.ComponentType<BrandIconProps>> = {
  github: GitHubIcon,
  cursor: CursorIcon,
  ...
  vscode: VSCodeIcon,
  mcp: McpIcon,
  'any ide via mcp': McpIcon,
};

export function BrandIcon({ name, ...props }: BrandIconProps & { name: string }) {
